...
describe("Interpreter", function() {
  ...
  it("should process a document with a lot of elements", function() {
    assert.equal(
      sibylline.render(
        "a|||(2018)b|(>2018)c|d|||ef|||(-2020)g|h|||ijk|||(<2030)l|(>2040)|m|||",
        2018
      ),
      "abefgijkl"
    );
  ...
  it("should process a realistic document", function() {
    assert.equal(
      sibylline.render(
        "We |||(<2020)will begin construction in|began construction in||| 2020.",
        2018
      ),
  ...
  it("should handle YYYY-MM", function() {
    assert.equal(sibylline.render("a|||(<2018-12)b|c|||d", "2018-11"), "abd");
  });

  it("should handle YYYY-MM relative to actual time", function() {
    assert.equal(sibylline.render("a|||(<2018-01-31)b|c|||d"), "acd");
  });
  ...
  it("should handle MM-DD relative to actual time", function() {
    assert.equal(sibylline.render("a|||(>02-20)b|c|||d"), "abd");
  });

  it("should handle MM", function() {
    assert.equal(sibylline.render("a|||(-08)b|c|||d", "2018-11-30"), "abd");
  });

  it("should handle MM relative to actual time", function() {
    assert.equal(sibylline.render("a|||(+08)b|c|||d"), "acd");
  });

  it("should handle an input time that's more specific than the condition time", function() {
    assert.equal(sibylline.render("a|||(2018)b|c|||d", "2018-11-30"), "abd");
  });

  it("should handle a condition time that's more specific than the input time", function() {
    assert.equal(sibylline.render("a|||(2018-11-30)b|c|||d", "2018"), "abd");
  });
